...
export const PostPage: FunctionComponent<IPostPage> = ({ post, randomPosts }) => {
  return (
    <main className="md:max-w-6xl  md:mx-auto">
      ...
      <Container className="prose-xl md:prose-2xl mx-auto" >
        <ReactMarkdown children={post.html} />
      </Container>

      <FooterPost post={post} randomPosts={randomPosts} />
...
const Header = ({ post: { title, subtitle, photos } }: IHeader) => {
  return (
    <>
      ...
      {photos.length !== 0 && <div className="relative w-full md:max-w-4xl md:mt-10 md:mx-auto mb-4">
        <div className="relative m-auto md:max-w-2xl">
          <img
            src={photos[0].image}
            alt={title}
        ...
        <div className="w-full m-auto">
          <div className="sm:w-9/12 mb-4 m-auto">
            ...
            <p className="z-10 text-grey text-small text-sm">bron: {photos[0].bron}</p>

            <p className="z-10 text-gray-800  text-lg font-medium italic">{photos[0].onderschrift}</p>
          </div>
        </div>
      </div>}
    </>
...
const FooterPost = ({ randomPosts, post: { tags } }: IFooterPost) => {
  return (
    <>
      <div className="m-4">

        {tags && <ul className="flex flex-wrap mb-6  mt-4">
          {tags.map((tag: string) => {
            return (
      ...
      <section>
        ...
        <div className="flex flex-wrap overflow-hidden my-4 mx-2 max-w-3xl">
          {randomPosts.map((post: FullPost, id) => <PostItem key={id} post={post} />)}
        </div>
      </section>
    </>
  )
